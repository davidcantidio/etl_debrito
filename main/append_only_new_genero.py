...
def run_etl_genero():
    ...
    source_sheet = "tiktokGenero"  # Nome da aba de origem contendo os dados de gênero
    ...
    plataforma = source_sheet.lower().replace("genero", "").strip()

    if plataforma == "meta":
        etl_class = MetaGeneroETL
        veiculo_nome = None  # Será inferido dinamicamente via Placement, se aplicável
    elif plataforma == "tiktok":
        etl_class = TikTokGeneroETL
        veiculo_nome = "Tiktok"
    elif plataforma == "linkedin":
        etl_class = LinkedinGeneroETL
        veiculo_nome = "Linkedin"
    elif plataforma == "pinterest":
        etl_class = PinterestGeneroETL
