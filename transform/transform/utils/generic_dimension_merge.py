...
from transform.transform.utils.normalize import no   # novo import
from transform.transform.utils.normalize import normalize_nome
...
def _read_sheet(sheet: str, dimension_col: str = None) -> pd.DataFrame:
    ...
    if dimension_col:
        if dimension_col not in df.columns:
            log.warning(
                f"A coluna de dimensão '{dimension_col}' não foi encontrada na planilha '{sheet}'!"
            )
        else:
            # Normaliza os valores da coluna de dimensão
            df[dimension_col] = df[dimension_col].apply(
                normalize_nome
            )  # Supondo que 'normalize_nome' seja uma função definida

    return df


# ------------------------------- LOGGING DE DISTRIBUIÇÃO ---------------------- #
DISTRIBUICAO_LOGS: Counter[str] = (
    Counter()
)  # Contador de casos especiais durante redistribuição
...
def load_and_prepare_meta_placement_data() -> pd.DataFrame:
    ...
    df = df.dropna(
        subset=["ad_id", "date"]
    )  # Garantir que as colunas 'ad_id' e 'date' não estejam vazias
...
def compute_impression_weights(
...
def merge_placement_and_dimension_data(
    df_dimension: pd.DataFrame, df_place: pd.DataFrame, dimension_col: str
) -> pd.DataFrame:
    """
    ...
    for col in required_columns_place:
        if col not in df_place.columns:
            raise ValueError(
                f"Coluna '{col}' não encontrada no DataFrame de placement!"
            )
...
def distribute_dimension_metrics(
    df_in: pd.DataFrame, dimension_col: str
) -> pd.DataFrame:
    """
    ...
    log.info(
        f"✅ distribute_dimension_metrics ({dimension_col}) — {len(df_out)} linhas"
    )
    return df_out
