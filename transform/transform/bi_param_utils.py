...
from transform.transform.utils.get_google_client import get_google_client
...
class BIParamLookup:
    ...
    def utm_start_end(self) -> Dict[str, Dict[str, str]]:
        ...
        """
        raw = self._map_columns(
            key_kw="utm_content", val_kws=["start", "end"], upper_keys=False
        )
    ...
    def fill_missing_start_end_from_utm(
        ...
        *,
        ...
        for idx, key in (
            out.get(coluna_utm, "").astype(str).str.strip().str.lower().items()
        ):
            if not key:
                continue
            # start
            if not str(out.at[idx, coluna_start]).strip():
                ...
                if novo:
                    ...
                    if write_back and coluna_start.lower() in header_lc:
                        ...
                        updates.append(
                            {"range": rowcol_to_a1(idx + 2, c), "values": [[novo]]}
                        )
            # end
    ...
    def get_criativo_mapping(self) -> Dict[str, str]:
        ...
        """
        raw = self._map_columns(
            key_kw="utm_content", val_kws=["taxonomy_ad_name"], upper_keys=False
        )
    ...
    def fill_utm_content_from_ad_name(
        ...
        *,
        ...
        if coluna_ad_name not in out.columns:
            log.debug(
                "[BIParamLookup] coluna '%s' ausente; skip fill_utm_content",
                coluna_ad_name,
            )
            return out
        ...
        inv_map = {v[0].strip().upper(): k for k, v in raw.items() if v[0]}
    ...
    def get_linkedin_ad_name_map(self) -> dict[str, str]:
        if not hasattr(self, "_li_ad_name_map"):
            ...
            if utm_col is None or name_col is None:
                logging.warning(
                    "Não achei colunas 'utm_content[_raw]' ou 'taxonomy_ad_name_social' em BI_PARAMETRIZAÇÃO."
                )
    ...
    def get_objective_map(self) -> Dict[str, str]:
        ...
        if utm_col is None or obj_col is None:
            log.warning(
                "Não achei colunas 'utm_content[_raw]' ou 'objective' em BI_PARAMETRIZAÇÃO."
            )
            return {}
        # build map
...
def fill_objective_from_bi(
    ...
    *,
    ...
    mask = df.get(objective_col, pd.Series("", index=df.index)).astype(
        str
    ).str.strip().eq("") & df[key_col].astype(str).str.strip().ne("")

    df.loc[mask, objective_col] = (
        df.loc[mask, key_col].astype(str).str.strip().map(obj_map).fillna("")
    )
    return df
...
def load_utm_mapping(creds_path: str, spreadsheet_id: str) -> Dict[str, Dict[str, str]]:
    """
