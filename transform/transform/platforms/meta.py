...
from transform.transform.utils.preview_links import determine_meta_ad_preview_link
...
def transform_meta(df: pd.DataFrame, lookup=None) -> pd.DataFrame:
    ...
    if any(
        c.strip().lower() in ("preview_link_ig", "preview_link_fb") for c in df.columns
    ):
